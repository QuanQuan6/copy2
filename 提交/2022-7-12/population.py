...
class population:
    ...
    def check(self):
        data = self.__get_data(self.best_MS, self.best_OS)
        if data['end_time'].max() != self.best_score:
            print('错误',data['end_time'].max(),self.best_score)
            self.best_score = data['end_time'].max()
